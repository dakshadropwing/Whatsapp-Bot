...
export default function Conversations() {
  ...
  const { data: convsData, isLoading: isConvsLoading } = useQuery({
    ...
    queryFn: () => conversationService.list({ status: filter === 'all' ? undefined : filter, search: search || undefined, per_page: 50 }),
  })

  const conversationsList = convsData?.data || []
  if (!activeId && conversationsList.length > 0) setActiveId(conversationsList[0].id)
  ...
  const { data: agentsData } = useQuery({ queryKey: ['agents'], queryFn: () => agentService.list({ per_page: 50 }) })
  const agentsList = agentsData?.data || []

  const resolveMutation = useMutation({
    mutationFn: (id: string) => conversationService.resolve(id),
    onSuccess: () => { toast.success('Conversation resolved'); qc.invalidateQueries({ queryKey: ['conversations'] }) },
    onError: () => toast.error('Failed to resolve conversation'),
  })

  const escalateMutation = useMutation({
    mutationFn: (id: string) => conversationService.escalate(id),
    onSuccess: () => { toast.success('Conversation escalated'); qc.invalidateQueries({ queryKey: ['conversations'] }) },
    onError: () => toast.error('Failed to escalate conversation'),
  })

  const assignMutation = useMutation({
    mutationFn: ({ id, agentId }: { id: string; agentId: string }) => conversationService.assign(id, { assigned_agent_id: agentId }),
    onSuccess: () => { toast.success('AI Agent assigned'); qc.invalidateQueries({ queryKey: ['conversations'] }) },
  ...
  const sendMessageMutation = useMutation({
    ...
    onSuccess: () => { setMessageText(''); toast.success('Message sent'); qc.invalidateQueries({ queryKey: ['messages', activeId] }); qc.invalidateQueries({ queryKey: ['conversations'] }) },
    onError: () => toast.error('Failed to send message'),
  })

  const handleSendMessage = () => {
    if (!messageText.trim() || !active) return
    sendMessageMutation.mutate({ phone: active.contact_phone, message: messageText })
  }

  return (
    <PageWrapper className="chat-container" style={{ margin: '-1.5rem', height: 'calc(100vh - var(--header-height))' }}>
      {/* Sidebar */}
      <div className="chat-sidebar">
        <div className="chat-sidebar-header">
          ...
          <InputGroup size="sm">
            <Form.Control
              ...
              style={{ borderRadius: '.75rem', fontSize: '.85rem' }}
            />
          </InputGroup>
          <div className="d-flex gap-1 mt-2 flex-wrap">
            {['all', 'active', 'waiting', 'bot_handling', 'human_handling', 'escalated', 'resolved'].map((f) => (
              <button
                ...
                style={{
                  fontSize: '.7rem', padding: '.2rem .6rem', borderRadius: '1rem',
                  background: filter === f ? 'linear-gradient(135deg, #25d366, #128c7e)' : 'rgba(255,255,255,.05)',
                  color: filter === f ? '#fff' : 'var(--text-muted)',
                  border: filter === f ? 'none' : '1px solid rgba(255,255,255,.06)',
                }}
                onClick={() => setFilter(f)}
              >
        ...
        {isConvsLoading ? (
          <div className="d-flex justify-content-center py-5"><Spinner animation="border" variant="success" size="sm" /></div>
        ) : conversationsList.length === 0 ? (
          <div className="text-center py-5 fs-sm" style={{ color: 'var(--text-muted)' }}>No conversations found.</div>
        ) : (
              ...
              <div key={c.id} className={`chat-item ${c.id === activeId ? 'active' : ''}`} onClick={() => setActiveId(c.id)}>
                ...
                <div className="chat-main">
                  <div className="d-flex justify-content-between align-items-center">
                    <span className="fw-semibold" style={{ fontSize: '.85rem' }}>{c.contact_name || c.contact_phone}</span>
                    <span style={{ fontSize: '.7rem', color: 'var(--text-muted)' }}>
                      {c.last_message_at ? new Date(c.last_message_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : ''}
                    </span>
                  </div>
                  <div className="d-flex justify-content-between align-items-center mt-1">
                    <span className="text-truncate" style={{ fontSize: '.78rem', maxWidth: 180, color: 'var(--text-muted)' }}>
                      {c.priority === 'high' ? '⚠️ High Priority' : 'Message thread'}
                    </span>
                    <Badge bg={statusColors[c.status] || 'secondary'} className="badge-status">{c.status.replace('_', ' ')}</Badge>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="chat-main-area">
        {active ? (
          <>
            <div className="d-flex justify-content-between align-items-center p-3" style={{ background: 'var(--card-bg-solid)', borderBottom: '1px solid var(--border-color)' }}>
            ...
            <div className="chat-messages">
              {isMessagesLoading ? (
                ...
                <div className="text-center py-5 fs-sm" style={{ color: 'var(--text-muted)' }}>No messages in this conversation.</div>
              ) : (
                  ...
                  <div key={msg.id} className={`chat-bubble ${msg.direction}`}>
                    ...
                    <div className="mt-1" style={{ fontSize: '.65rem', color: msg.direction === 'outbound' ? 'rgba(255,255,255,.7)' : 'var(--text-muted)' }}>
                      ...
                      {msg.direction === 'outbound' && (
                        <span> &middot; <i className={`bi ${msg.status === 'read' ? 'bi-check2-all' : 'bi-check2'}`} title={msg.status} /></span>
                      )}
            ...
            <div className="chat-input-area">
              <button className="btn btn-sm btn-light" style={{ borderRadius: '50%', width: 36, height: 36 }} aria-label="Attach file"><i className="bi bi-paperclip" /></button>
              <Form.Control
          ...
          <div className="d-flex flex-column align-items-center justify-content-center h-100">
            <div className="empty-state">
              ...
              <p className="fs-sm" style={{ color: 'var(--text-muted)' }}>Choose a conversation from the sidebar to start chatting</p>
            </div>
          </div>
        )}
      </div>
