    ...
    <PageWrapper>
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <div className="d-flex gap-2">
          <select className="form-select form-select-sm" style={{ width: 140, borderRadius: '.75rem' }}><option>All Actions</option><option>Auth</option><option>Settings</option></select>
          <button className="btn btn-sm btn-light"><i className="bi bi-download me-1" />Export</button>
        </div>
      </div>
      <div className="data-card">
        <div className="table-responsive"><table className="table table-custom mb-0"><thead><tr><th>Action</th><th>User</th><th>Resource</th><th>Detail</th><th>Time</th></tr></thead>
          <tbody>{logs.map((l) => (
            <tr key={l.id}>
              ...
              <td className="fs-sm" style={{ color: 'var(--text-muted)' }}>{l.detail}</td>
              <td style={{ fontSize: '.8rem', color: 'var(--text-muted)' }}>{l.time}</td>
            </tr>
          ))}</tbody>
        </table></div>
      </div>
