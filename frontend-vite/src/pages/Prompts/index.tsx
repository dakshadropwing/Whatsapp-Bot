  ...
  return (
    <PageWrapper>
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <button className="btn btn-wa-primary" onClick={() => setShowModal(true)} aria-label="Create new prompt template">
                        ...
                        {p.variables && p.variables.length > 0 ? (
                          p.variables.map((v: string) => (
                            <code
                              ...
                              style={{ fontSize: '.7rem', background: 'rgba(37,211,102,.1)', padding: '1px 6px', borderRadius: 4 }}
                            >
                              {v}
                            </code>
                          ))
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Form onSubmit={handleSubmit}>
          ...
          <Modal.Footer>
            ...
            <Button type="submit" className="btn-wa-primary" disabled={createMutation.isPending}>{createMutation.isPending ? 'Saving...' : 'Create Prompt'}</Button>
          </Modal.Footer>
        </Form>
      </Modal>
    </PageWrapper>
